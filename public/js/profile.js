...
if (profileForm) {
  profileForm.addEventListener("submit", (e) => {
    ...
    if (!validateName(firstNameInput.value.trim()))
      errors.push(
        "firstName must be between 2 and 25 characters, and cannot contain any numbers",
      );
    if (!validateName(lastNameInput.value.trim()))
      errors.push(
        "lastName must be between 2 and 25 characters, and cannot contain any numbers",
      );
    if (
    ...
    if (ageVal === "" || isNaN(ageNum) || (ageNum < 0 && !isNaN(ageNum)))
